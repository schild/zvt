    ...
    while error_count <= 10:
        ...
        try:
            ...
            tech_factor = factor_cls(entity_schema=entity_schema, entity_provider=entity_provider,
                                     ...
                                     start_timestamp=start_timestamp, end_timestamp=target_date,
                                     adjust_type=adjust_type, **factor_kv)
...
def report_top_stats(entity_provider,
                     data_provider,
    ...
    entity_schema = get_entity_schema(entity_type=entity_type)
    latest_day = kdata_schema.query_data(provider=data_provider, order=kdata_schema.timestamp.desc(), limit=1,
                                         return_type='domain')
    ...
    if ignore_new_stock:
        ...
        entity_ids = get_entity_ids(provider=entity_provider, entity_schema=entity_schema,
    ...
    for period in periods:
        ...
        df, _ = get_top_performance_entities(entity_type=entity_type, start_timestamp=start, filters=filters, pct=1,
                                             show_name=True, entity_provider=entity_provider,
